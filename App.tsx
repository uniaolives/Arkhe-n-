...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'biogenesis' | 'parallax' | 'quantum' | 'fusion' | 'metasurface' | 'arkhe' | 'twin'>('biogenesis');
  ...
  const [messages, setMessages] = useState<EchoMessage[]>([
    ...
    {
      id: 'sensorium-init',
      sender: 'ARKHOS_SENSORIUM',
      content: 'LIDAR, THERMAL, AND DEPTH KERNELS ACTIVE. FUSION COHERENCE OPTIMIZING.',
      timestamp: new Date().toISOString(),
      year: 2026,
      type: 'event'
    }
      ...
      <header className="flex justify-between items-center border border-indigo-500/20 p-3 rounded-xl backdrop-blur-xl z-10 relative bg-black/40 shadow-lg">
        <div className="flex gap-4 items-center">
          ...
          <div>
            ...
            <p className="text-[7px] mt-1 opacity-60 uppercase tracking-widest font-bold text-indigo-300">
              CLUSTER: ARKHEN-ALPHA-26 // STATUS: {status} // MODE: {activeTab.toUpperCase()}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2 items-center">
           ...
           <div className="flex gap-1 bg-black/60 p-1 rounded border border-white/10">
             ...
             <button onClick={() => setActiveTab('metasurface')} className={`px-2 py-1 text-[8px] font-black rounded transition-all ${activeTab === 'metasurface' ? 'bg-amber-600 text-black shadow-[0_0_10px_amber]' : 'text-amber-500/50 hover:text-amber-500'}`}>SURFACE</button>
